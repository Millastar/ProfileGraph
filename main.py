...
import os
# 用于将同步 LangGraph 消息流桥接到异步 SSE 响应
import asyncio
import queue
import threading
# 用于正则表达式匹配和处理字符串
...
async def handle_stream_response(user_input, graph, config):
    ...
    async def generate_stream():
        ...
        try:
            ...
            def produce_graph_stream():
                """Run the synchronous graph on one worker thread and forward its events."""
                try:
                    ...
                    for stream_item in stream_data:
                        if stop_event.is_set():
                            break
                        stream_queue.put(("chunk", stream_item))
                except Exception as producer_error:
                    stream_queue.put(("error", producer_error))
                finally:
                    stream_queue.put(("done", None))

            producer_thread = threading.Thread(
                target=produce_graph_stream,
                name=f"graph-stream-{chunk_id[-8:]}",
                daemon=True,
            )
            ...
            # 遍历消息流中的每个数据块
            while True:
                # queue.get 在工作线程中等待，Uvicorn 事件循环可继续刷新网络数据。
                event_type, event_value = await asyncio.to_thread(stream_queue.get)
                if event_type == "done":
                    break
                if event_type == "error":
                    raise event_value

                message_chunk, metadata = event_value
                try:
                    # 获取当前节点名称
                    ...
                    if node_name in ["generate", "agent"]:
                        ...
                        if chunk:
                            chunk_count += 1
                            # 产出流式数据块；不要逐 token 写同步文件日志，否则会拖慢发送。
            ...
            logger.info("Stream completed with %s content chunks", chunk_count)
            # 产出流结束标记
            ...
            # 记录流生成过程中的异常
            logger.exception("Stream generation error: %r", stream_error)
            # 产出错误提示
